...
class UniMatch(nn.Module):
    ...
    def extract_feature(self, img0, img1):
        ...
        # Ensure img0 and img1 are tensors for concatenation
        if isinstance(img0, tuple):
            img0 = img0[0]  # Assuming the tensor is the first element of the tuple
        if isinstance(img1, tuple):
            img1 = img1[0]
        print(type(img0))  # Prints the type of img0
    ...
    def forward(self, img0, img1,
                attn_type='self_swin2d_cross_swin1d',    # Defaulted values to reduce conditional checks
        ...
        if task == 'stereo':
            return flow_preds[-1].squeeze(1)  # [B, H, W] for stereo
        elif task == 'depth':
            return 1. / flow_preds[-1].squeeze(1)  # [B, H, W] for depth
        else:
            return flow_preds[-1]  # Return [B, 2, H, W] for flow
