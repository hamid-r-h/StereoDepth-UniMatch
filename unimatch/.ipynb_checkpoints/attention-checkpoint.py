...
def single_head_split_window_attention_1d(q, k, v,
    ...
    if with_shift:
        assert attn_mask is not None  # compute once
        # Ensure window_size_w is an integer
        shift_size_w = window_size_w // 2
        
        # Check if shift_size_w is a tensor, convert to int if necessary
        if isinstance(shift_size_w, torch.Tensor):
            shift_size_w = int(shift_size_w.item())
        else:
            shift_size_w = int(shift_size_w)  # Just to be safe if it isn't an integer
...
class SelfAttnPropagation(nn.Module):
    ...
    def forward_local_window_attn(self, feature0, flow,
        ...
        kernel_size = kernel_size.item() if isinstance(kernel_size, torch.Tensor) and kernel_size.dim() == 0 else kernel_size
        local_window_radius = local_window_radius.item() if isinstance(local_window_radius, torch.Tensor) and local_window_radius.dim() == 0 else         local_window_radius
