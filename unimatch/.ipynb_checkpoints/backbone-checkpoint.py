...
class CNNEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Use torch.jit.annotate to declare the type explicitly
        out = annotate(List[torch.Tensor], [])

        if self.num_branch > 1:
            # Ensure that out is treated as a List[Tensor]
            out = self.trident_conv([x] * self.num_branch)  # High to low resolution
        else:
            # Wrap x in a list if out expects a list type
            out = [x]
